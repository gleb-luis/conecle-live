...
class QuoteController extends BaseController
{
  ...
  public function saveQuoteProduct(Request $request) {
    QuoteProduct::create([
      ...
      'tax' => $request->has('tax')?$request->input('tax'):1,
    ]);
  ...
  public function getQuoteDetailInfo(Request $request) {
    $quote = Quote::where('id', $request->input('id'))->with(['products','recipienter'])->first();
    $quote['owner'] = $quote->recipienter?$quote->recipienter->owner:'';
    return response()->json([
      'quote' => $quote
    ]);
  }
  ...
  public function quoteDeleteProc(Request $request) {
    return response()->json([
      'flag' => (bool)Quote::query()->where('id', $request->input('id'))->delete()
    ]);
  }
