<template>
  <div class="order__detail_container">
    <div class="d-flex heading">
      <h2 class="page__title">請求書</h2>
    </div>
    <div class="status__part" v-if="!!order_form">
      <div class="panel">
        <h3 class="page__title">書類情報</h3>
      ...
      <div class="status__action">
        <button @click="downloadPDF"><img src="/asset/icons/pdf.svg" />請求書作成</button>
      </div>
    </div>
    ...
    <div class="client__info" v-if="!!order_form">
      ...
      <div class="right">
        <div class="info__row">
          <label>請求書番号：</label>
          <p>{{ order_form.invoice_hash }}</p>
        </div>
        <div class="info__row">
          <label>作成日：</label>
          <p>{{ order_form.invoice_at | dateFormat }}</p>
        </div>
        <div class="info__row">
          <label>支払期限：</label>
          <p v-if="!isEditing">{{ pay_expire | dateFormat }}</p>
          <input type="date" v-else v-model="pay_expire" @change="changePayExpire" />
          <button @click="editExpire">{{ isEditing ? '保存' : '編集' }}</button>
        </div>
        ...
        <div class="address">
          担当者：{{ order_form.recipienter.owner || '管理者' }}
        </div>
        <div class="address">
        ...
        <section class="pdf-item">
          <div class="pdf_container">
            <div class="pdf_heading">
              <p class="contact_to">{{ order_form.user.com_name }}　御中</p>
              <div class="page_type">
                <p>請求書</p>
                <p>作成日：{{ order_form.invoice_at | dateFormat }}　請求書番号：{{ order_form.invoice_hash || '' }}<br/>支払い期日：{{ order_form.pay_expire | dateFormat }}</p>
              </div>
            </div>
            <div class="order_detail_info">
              <div class="left">
                <div class="d-flex">
                  <label>請求額</label>
                  <p v-if="order_form.tax_type == 1">￥{{ parseInt(calcTotalPrice() + calcTaxFee8P() + calcTaxFee10P()) | moneyFormat }}（税込）</p>
                  <p v-if="order_form.tax_type == 2">￥{{ parseInt(calcPrice8P() + calcPrice10P()) | moneyFormat }}（税込）</p>
                  <p v-if="order_form.tax_type == 3">￥{{ calcTotalPrice() | moneyFormat }}（税込）</p>
                </div>
              </div>
              <div class="right">
		    ...
		    {{ $store.getters['recipienter_auth/user'].owner }}<br/>
  ...
  methods: {
    ...
    async editExpire() {
      ...
      if (this.isEditing == false) {
        if (!!this.pay_expire) {
          try {
            const { data } = await axios.post('/recipienter/save_pay_expire', {
    ...
    async okFinishDelivery() {
      try {
        const { data } = await axios.post('/recipienter/send_invoice_email', {
    ...
    sendInvoiceEmail() {
      this.$router.push({ name: 'recipient.invoice.message.create', query: { id: this.$route.query.id } })
    },
    downloadPDF() {
    ...
    async init() {
      try {
        ...
        this.pay_expire = data.order_form.pay_expire
