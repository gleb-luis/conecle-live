        ...
        <section class="pdf-item" v-for="(page, index) in calcSelectedRows()" :key="'page' + index">
          <div class="pdf_container">
            <div class="pdf_heading">
              <p class="contact_to">{{ page.user.com_name }}<br/>{{ page.user.name }}御中</p>
              <div class="page_type">
                <p>納品書</p>
		    ...
		    {{ $store.getters['recipienter_auth/user'].owner }}<br/>
                    ...
                    担当者：{{ $store.getters['recipienter_auth/user'].tanto_name || 'マスタアカウント' }}<br/>
          ...
          <div class="pdf_container">
            ...
            <div class="total_price">
              <table>
                <tr>
                  <td>小計(税抜)</td>
                  <td>￥{{ parseInt(page.price / 1.1) | moneyFormat }}</td>
                </tr>
                <tr>
                  <td>消費税</td>
                  <td>￥{{ page.tax == 1 ? parseInt(page.price /1.1) * 0.1 : parseInt(page.price /1.1) * 0.08 | moneyFormat }}</td>
                </tr>
                <tr>
                  <td>合計</td>
                  <td>￥{{ page.tax == 1 ? parseInt(page.price / 1.1) + parseInt(page.price /1.1) * 0.1 : parseInt(page.price /1.1) + parseInt(page.price /1.1) * 0.08 | moneyFormat }}</td>
                </tr>
              </table>
...
export default {
  ...
  methods: {
    ...
    downloadCSVConfirm() {
      ...
      window.open('/recipienter/orders/export/csv/'+this.selectedIds+'/'+this.$store.getters['recipienter_auth/user'].id,  '_blank');
    },
    createMultiInvoice() {
      if (this.calcSelectedRows().length == 0) {
        this.$swal('エラー', 'チェックしたものがありません。')
