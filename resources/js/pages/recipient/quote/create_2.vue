  ...
  <div class="order__detail_container">
    ...
    <div class="client__info" v-if="step == 2">
      <div class="left">
        ...
        <p class="total_price">
          <span>合計金額</span>
          <span>￥{{ parseInt(calcSubTotalPrice() + calcTaxFee8P() + calcTaxFee10P() ) | moneyFormat }}（税込）</span>
        </p>
      </div>
      <div class="right">
        <div class="info__row">
    ...
    <div class="detail__part">
        ...
        <vue-good-table
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <div v-else-if="props.column.field == 'tax'">
              <input class="price" :checked="props.row.tax == 2" type="checkbox" @change="changeTax($event, props.index)" :disabled="step == 2" />
            </div>
      ...
      <div class="append__part">
        <button @click="appendRow" :disabled="step == 2">行を追加</button>
      </div>
      <div class="total__part">
        ...
        <div class="calculator">
          <div class="calc_row">
            <label>小計（税抜）</label>
            <p>￥{{ calcSubTotalPrice() | moneyFormat }}</p>
          </div>
          <div class="calc_row">
            <label>消費税（8％）</label>
          ...
          <div class="calc_row">
            <label>合計</label>
            <p>￥{{ parseInt(calcSubTotalPrice() + calcTaxFee8P() + calcTaxFee10P() ) | moneyFormat }}</p>
          </div>
...
export default {
  ...
  mounted() {
    this.subject_title = localStorage.getItem('subject_title')
    this.publish_date = localStorage.getItem('publish_date')
  ...
  methods: {
    ...
    async saveProc() {
      ...
      try {
        const { data } = await axios.post('/recipienter/save_quote', {
        ...
        if (!!data.quote) {
          this.saveQuoteProduct(data.quote)
        }
      } catch (error) {
      }
    },
    async saveQuoteProduct(quote) {
