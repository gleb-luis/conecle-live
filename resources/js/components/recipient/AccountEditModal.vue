      ...
      <div class="account_form">
        ...
        <div class="form__row flex-start">
          ...
          <div class="group">
            ...
            <p>
              <input type="text" placeholder="番地" v-model="address" />
            </p>
          </div>
        </div>
        <div class="form__row">
...
export default {
  ...
  mounted() {
    this.name = this.$store.getters['recipienter_auth/user'].name
    this.owner = this.$store.getters['recipienter_auth/user'].owner
    this.zipcode = this.$store.getters['recipienter_auth/user'].zipcode
    this.prefecture = this.$store.getters['recipienter_auth/user'].prefecture
    this.city = this.$store.getters['recipienter_auth/user'].city
    this.address = this.$store.getters['recipienter_auth/user'].address
    this.phone = this.$store.getters['recipienter_auth/user'].phone
  },
  methods: {
    closeModal() {
    ...
    async save() {
      try {
        const { data } = await axios.post('/recipienter/edit_account_info', {
          name: this.name,
          owner: this.owner,
